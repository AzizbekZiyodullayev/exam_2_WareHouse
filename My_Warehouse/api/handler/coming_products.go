...
func (h *handler) CreateComingProducts(c *gin.Context) {
	...
	id, err := h.storage.ComingProducts().Create(c.Request.Context(), &createComingProducts)
	if err != nil {
		h.handlerResponse(c, "storage.comingProducts.create", http.StatusInternalServerError, err.Error())
		return
	}

	resp, err := h.storage.ComingProducts().GetByID(c.Request.Context(), &models.ComingProductsPrimaryKey{Id: id})
	if err != nil {
...
func (h *handler) GetListComingProducts(c *gin.Context) {
	...
	resp, err := h.storage.ComingProducts().GetList(c.Request.Context(), &models.ComingProductsGetListRequest{
		Offset:             offset,
		Limit:              limit,
...
func (h *handler) UpdateComingProducts(c *gin.Context) {
	...
	rowsAffected, err := h.storage.ComingProducts().Update(c.Request.Context(), &updateComingProducts)
	if err != nil {
	...
	resp, err := h.storage.ComingProducts().GetByID(c.Request.Context(), &models.ComingProductsPrimaryKey{Id: updateComingProducts.Id})
	if err != nil {
		h.handlerResponse(c, "storage.comingProducts.getById", http.StatusInternalServerError, err.Error())
		return
	}
...
func (h *handler) DeleteComingProducts(c *gin.Context) {
	...
	err := h.storage.ComingProducts().Delete(c.Request.Context(), &models.ComingProductsPrimaryKey{Id: id})
	if err != nil {

...
func (h *handler) CreateComing(c *gin.Context) {
	...
	id, err := h.storage.Coming().Create(c.Request.Context(), &createComing)
	if err != nil {
		h.handlerResponse(c, "storage.Coming.create", http.StatusInternalServerError, err.Error())
		return
	}

	resp, err := h.storage.Coming().GetByID(c.Request.Context(), &models.ComingPrimaryKey{Id: id})
	if err != nil {
...
func (h *handler) GetListComing(c *gin.Context) {
	...
	resp, err := h.storage.Coming().GetList(c.Request.Context(), &models.ComingGetListRequest{
		Offset:             offset,
		Limit:              limit,
		Search_by_comingID: c.Query("search_by_comingID"),
		Search_by_branch:   c.Query("search_by_branch"),
	})
	if err != nil {
...
func (h *handler) UpdateComing(c *gin.Context) {
	...
	updateComing.Id = id
	rowsAffected, err := h.storage.Coming().Update(c.Request.Context(), &updateComing)
	if err != nil {
	...
	resp, err := h.storage.Coming().GetByID(c.Request.Context(), &models.ComingPrimaryKey{Id: updateComing.Id})
	if err != nil {
		h.handlerResponse(c, "storage.Coming.getById", http.StatusInternalServerError, err.Error())
		return
	}
...
func (h *handler) DeleteComing(c *gin.Context) {
	...
	err := h.storage.Coming().Delete(c.Request.Context(), &models.ComingPrimaryKey{Id: id})
	if err != nil {
